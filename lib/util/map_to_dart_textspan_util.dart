...
import 'package:json_to_dart/widget/dart_output_area.dart';
...
class MapToDartTextSpanUtil {
  static TextSpan trans(Map map, {required String name, required bool needDecodeFunction}) {
    ...
    try {
      ...
      // 成员属性区
      FieldParserTextSpanResult fieldParserResult = _fieldArea(map, name, needDecodeFunction);
      ...
      // fromJson函数
      ts += _fromJsonFunctionArea(map, name);
  ...
  static FieldParserTextSpanResult _fieldArea(Map map, String name, bool needDecodeFunction) {
    TextSpan ts = const TextSpan();
    ...
    for (var key in keyList) {
      ...
      if (!_judgeIncorrectKey(key)) {
        ...
        if (value.isEmpty) {
          ts += const TextSpan(text: '${_baseSpaces}Map? ');
          ...
          clzs.add(trans(value, name: "$name$keyCapitalize", needDecodeFunction: needDecodeFunction));
        }
      } else if (value is List) {
        if (value.isEmpty) {
          ts += const TextSpan(text: '${_baseSpaces}List? ');
          ts += TextSpan(text: '$key', style: fieldStyle);
          ts += const TextSpan(text: ';\n');
        } else {
          ...
          if ('$listItemType'.contains('Map')) {
            ...
            clzs.add(trans(firstItem as Map, name: listItemType, needDecodeFunction: needDecodeFunction));
  ...
  static TextSpan _fromJsonFunctionArea(Map map, String name) {
    TextSpan ts = const TextSpan();

    ts += const TextSpan(text: '\n$_baseSpaces');

    ts += TextSpan(text: "${name}Model", style: classNameStyle);
    ...
    map.keys.toList().forEach((key) {
      ...
      if (value is Map) {
        if (value.isEmpty) {
          ...
          ts += TextSpan(text: key, style: fieldStyle);
          ts += TextSpan(
              text: ' = json[\'$key\'] != null ? $name${keyCapitalize}Model.fromJson(json[\'map\']) : null;\n');
        }
      } else if (value is List) {
        if (value.isEmpty) {
          ts += const TextSpan(text: '$_baseSpaces  ');
        ...
        if (json['$key'] is List) {
            ''');
            ts += TextSpan(text: key, style: fieldStyle);
            ts += TextSpan(text: ''' = json["$key"] == null
            ? null 
